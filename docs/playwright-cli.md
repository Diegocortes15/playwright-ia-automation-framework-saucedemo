...
> **Why every command here says `npx`.** The binary is not on `PATH` — it ships in
> `node_modules/.bin/`, so bare `playwright-cli` fails with `command not found`. You will see the
> bare form throughout `.claude/skills/playwright-cli/SKILL.md`; that file is vendored from
> `@playwright/cli` and regenerated by `npx playwright-cli install --skills`, so it is not ours to
> correct. `CLAUDE.md` carries the standing correction instead, where regeneration cannot reach it,
> and `scripts/check-doc-commands.mjs` fails the build if a bare invocation creeps back in.
...
npx playwright-cli fill <ref-username> standard_user
npx playwright-cli fill <ref-password> secret_sauce
npx playwright-cli click <ref-login>
...
npx playwright-cli generate-locator <ref>     # Confirms playwright generates the expected locator
npx playwright-cli click <ref>                # Confirms it's clickable (or fails loudly)
```
...
npx playwright-cli eval "el => el.outerHTML" <ref>
npx playwright-cli eval "el => ({ visible: el.offsetParent !== null, text: el.innerText })" <ref>
```
