...
- **One committed artifact; every readable view is derived.** `.observations/observations.json` is the record and the only thing in version control. The prose digest renders on demand with `npm run observations`, the per-test view is the Playwright report, and the reviewer's view is generated into the PR body by `/from-issue`. An earlier version committed a generated `SUMMARY.md` beside the JSON: a derived file in version control is diff noise, a merge-conflict surface, and a chance for the copy to drift from its source. The case for committing it — "a reviewer should read prose" — is already served by the PR body.
