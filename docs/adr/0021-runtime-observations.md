...
- **The icon carries severity, not category.** The group heading already says whether something is Network, Console or a Dialog, so the emoji says how bad it is instead: 🔥 for 5xx, 🔒 for 401/403, 🔍 for 404, ⏳ for throttling, 💥 for an uncaught exception, ❗ for a console error, 💬 for a dialog. A 500 from your own server and a 404 on a stale link are both `failed-request` and are not remotely the same problem; one glance should separate them. Same function drives the report chips, the attachments and SUMMARY.md.
