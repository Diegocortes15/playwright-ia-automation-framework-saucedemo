...
- **One index keyed by signature, not one file per feature.** An observation is a fact about the application, so it is stored once with a `seenIn` list of the features that trip it. The first version split by feature and duplicated everything: the same 404 appeared in six files and had to be triaged six times, and a new feature hitting a known signature would have come back `new` again — the decision would have to be re-made forever. Triage is a decision about a signature, so the file is keyed by one.
- **Two artifacts, because one cannot serve both readers.** `.observations/observations.json` is a machine index — signatures, counts, triage state — built to deduplicate and to diff in git. `.observations/SUMMARY.md` is the same data as prose, regenerated every run, so "what did the app do?" is answered by reading a sentence instead of parsing a colon-delimited key. One template per kind, no LLM. The first version shipped only the JSON and called it human-readable; it was not.
- **In the Playwright report, grouped the way devtools groups them.** Each test carries one `text/plain` attachment per group — `observations — Network`, `— Console`, `— Dialogs` — rendered inline, so the prose reads without downloading anything, and a chip per group whose annotation _type_ is the label, so Network and Console separate visually. `page-error` sits with Console because an uncaught exception surfaces there. A group with one entry puts the sentence straight in the chip; more than one points at its attachment.
- **The icon carries severity, and only four of them.** The group heading already says whether something is Network, Console or a Dialog, and the sentence underneath already gives the precise status — "rejected as unauthorised", "was not found on the server". So the icon carries the one distinction that changes what you do: 🔥 the application broke (5xx or an uncaught exception), ⚠️ it refused or lacked something (any 4xx), ❗ a console error, 💬 a dialog. A first pass had eight, splitting 401 from 404 from 429, and needed a legend to be readable — which is the tell that the encoding was repeating the text one line below it. One function drives the report chips, the attachments and SUMMARY.md, so the same event never reads differently depending on where it is found.
