...
// Augmented by: SW-4 (2026-05-27), SW-5 (2026-05-30), SW-901 (2026-09-06)
...
test.describe('inventory — standard_user', { tag: '@standard' }, () => {
  test.describe('Positive', () => {
    ...
    // SW-901 — the header cart badge reflects how many products are in the cart.
    // saucedemo renders no badge at all for an empty cart, so getCartBadgeCount()
    // reports 0 for "absent" (see CartBadge).
    test('cart badge counts each product added from the inventory page', async ({
      inventoryPage,
    }) => {
      await inventoryPage.goto();
    ...
    test('cart badge disappears when the last product is removed', async ({ inventoryPage }) => {
      await inventoryPage.goto();
      await inventoryPage.addToCart(products[0].name);
      expect(await inventoryPage.getCartBadgeCount()).toBe(1);

      await inventoryPage.removeFromCart(products[0].name);
      expect(await inventoryPage.getCartBadgeCount()).toBe(0);
    });
  });
