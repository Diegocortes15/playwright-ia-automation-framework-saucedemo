...
Projects are **data-driven from `tests/users.ts` `AUTH_USERS`** (currently `['standard', 'problem']`); `/from-issue` grows that array one user at a time as tickets need authenticated pages (ADR-0014). Each user yields a `setup-<user>` + a `chromium-<user>` project; plus a `chromium-no-auth` project. Cross-browser is **implemented and opt-in** ([ADR-0027](docs/adr/0027-cross-browser-opt-in.md)): `npm test` stays chromium, and `npm run test:firefox` / `test:webkit` / `test:cross` set `CROSS_BROWSER=1` to add the four Firefox/WebKit projects. Standard user only — never a per-user × per-browser matrix.
