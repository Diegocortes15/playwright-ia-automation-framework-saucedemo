...
- **`/report-bug`** — turn a failed Playwright run into a bug report draft: repro steps (the `test.step` titles the Page Objects already wrap actions in), the acceptance criterion the test traces to, expected vs actual, correlated runtime observations, and evidence paths. **Files nothing** — you invoke it, read the draft, and decide. It never resolves which of three things is wrong — the _application_, the _ticket_, or our own _automation_ — not even when `docs/app/` covers the behaviour, because documentation records known defects as readily as intended behaviour. All three readings stay in the draft, weighed by the build under test the run recorded; a person settles it.
...
Projects are **data-driven from `tests/users.ts` `AUTH_USERS`** (currently `['standard', 'problem', 'error']`); `/from-issue` grows that array one user at a time as tickets need authenticated pages (ADR-0014). Each user yields a `setup-<user>` + a `chromium-<user>` project; plus a `chromium-no-auth` project. Cross-browser is **implemented and opt-in** ([ADR-0027](docs/adr/0027-cross-browser-opt-in.md)): `npm test` stays chromium, and `npm run test:firefox` / `test:webkit` / `test:cross` set `CROSS_BROWSER=1` to add the four Firefox/WebKit projects. Standard user only — never a per-user × per-browser matrix. In CI it is reachable **on manual dispatch only** ([ADR-0029](docs/adr/0029-cross-browser-on-demand-in-ci.md)): `regression.yml` takes a `browser` input, the scheduled cadences are pinned to chromium, and `scripts/run-suite.sh` exits 66 rather than let a cron run another engine.

| Tag          | Runs on project(s)                                                                  | Purpose                                                                                                                                                                                                                            |
| ------------ | ----------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `@no-auth`   | `chromium-no-auth`                                                                  | Login / logout / route-guard tests — no pre-existing session                                                                                                                                                                       |
| `@all-users` | every `chromium-<user>` project (currently `chromium-standard`, `chromium-problem`) | User-agnostic flows. **Available, and used by zero specs today** — every test is pinned to one user. `footer` is the natural first candidate: a copyright line should read the same for anybody                                    |
...
| `@<user>`    | `chromium-<user>` — **only once that user is wired** into `AUTH_USERS`              | On-demand per ADR-0014: e.g. `@performance_glitch` / `@error` / `@visual` route nowhere until a ticket needs that user (then `/from-issue` adds it + its project). Cross-browser covers the standard user only, opt-in (ADR-0027). |
...
| Build under test (what a run ran _against_)     | `src/utils/app-build.ts` + `global-setup.ts`; written to `test-results/app-build.json`, shown as the report's `App` chip. Declare one with `APP_BUILD` |
